...
结合上面两点可以得到Lambertian diffuse shading公式：$$ L_{d}=k_{d}(I/r^2)max(0,n \cdot l) $$  
...
所以对于任何一个点，有$$L = L_{a}+L_{d}+L_{s} = k_{a}I_{a}+k_{d}(I/r^2)max(0,n \cdot l)+k_{s}(I/r^2)max(0,n \cdot h)^p $$  
