...
视图变换view transform的作用就是对空间中任意一个相机，把它的位置重置在参考系原点(平移)，向上方向对齐y轴，看向-z方向(旋转)。**同时对空间中的其他所有物体也应用这个变换**，这样才能保持物体和相机之间的相对位置保持不变。

View变换的本质就是一个旋转矩阵和一个平移矩阵，先平移再旋转 （不能直接写成一个齐次仿射变换，齐次仿射变换是先旋转后平移）
...
* 通过平移将场景移动到世界中心。
* 将场景归一化到[-1, 1]^3的空间内。
* 舍弃z轴，就得到一个二维的平面。但是我们保留z轴，当做深度信息用作深度排序。
...
$$
M_{persp} = M_{persp→ortho}\times M_{ortho} = \begin{pmatrix}
n & 0 & 0 & 0\\
0 & n & 0 & 0\\
0 & 0 & n+f & -nf\\
0 & 0 & 1 & 0\\
