...
# 其他
## 漫反射半球采样
主要是Material类里的sample函数  
该函数给定一个入射方向和交点法线，返回一个随机的反射方向。
 
三维极坐标中，一个点的位置由 极角，方位角，和球半径据决定。 
我们在[0,1]取随机数x1,x2。 
我们把x1当作极角采样，x2当作方位角采样。


# BVH相交流程
* 光线与整个场景的BVH进行相交测试，一直找到最近相交的**叶节点(存储物体)**，如果找不到叶子节点，返回空相交。
* 光线与最小叶节点中的物体的BVH进行相交测试，找到最近相交的**叶节点(存储三角形)**，如果找不到叶子节点，返回空相交。
* 光线与最小叶子节点中的三角形进行相交测试，计算并返回是否相交，交点坐标，交点法线等信息。
